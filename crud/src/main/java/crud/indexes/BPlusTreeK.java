...
public class BPlusTreeK implements SystemSpecification {
    ...
    public BPlusTreeK(int o, String na) throws IOException {
    ...
    public void toJsonFile() throws IOException {
        ...
        String[] strs = this.nomeArquivo.split("/");

        arquivo.seek(0);
        long root = arquivo.readLong();
    ...
    private void toJsonFile(StringBuffer sb, long address) throws IOException {
        arquivo.seek(address);
        Pagina curr = new Pagina(ordem);
        byte[] buffer = new byte[curr.TAMANHO_PAGINA];
        arquivo.read(buffer);
        curr.setBytes(buffer);

        if(curr != null) {
            toJsonFile(sb, curr.filhos[0]);
            sb.append(curr.toString() + ",\n");
            
            for(int i = 1; i < this.ordem; i++)
                toJsonFile(sb, curr.filhos[i]);
        }
    }
